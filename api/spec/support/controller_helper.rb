module ControllerHelper
  ...
  def get_as role_or_record, url, opts={params: {}, headers: {}, body: ""}
    allow_any_instance_of(AuthorizeApiRequest).to receive(:user).and_return user_object(role_or_record)
    get url, opts
  end

  def delete_as role_or_record, url, opts={params: {}, headers: {}, body: ""}
    allow_any_instance_of(AuthorizeApiRequest).to receive(:user).and_return user_object(role_or_record)
    delete url, opts
  end

  def post_as role_or_record, url, opts={params: {}, headers: {}, body: ""}
    allow_any_instance_of(AuthorizeApiRequest).to receive(:user).and_return user_object(role_or_record)
    post url, opts
  end

  def put_as role_or_record, url, opts={params: {}, headers: {}, body: ""}
    allow_any_instance_of(AuthorizeApiRequest).to receive(:user).and_return user_object(role_or_record)
    put url, opts
  end
